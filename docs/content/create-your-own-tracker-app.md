...
This how-to explains how you can create your own Tracker app using Mendix Studio. You can use this Tracker app to keep track of everything from office supplies to warehouse stock.
...
* You want to control the stock via an app
...
* [Configure the user interaction elements of *Parts* page](https://github.com/RaviBasagonda/mendix/blob/main/docs/content/create-your-own-tracker-app.md#6-configuring-the-user-interaction-elements-of-parts-page)
  ...
  * Create and configure buttons, pages, and microflows to add a new part, edit stock, or delete a part from the parts list
  * Create added features such as search, sort options, and filter options to control the parts stock
...
To create and start editing the Tracker app in Mendix Studio, follow these steps:

1. Sign in to your Mendix account.
2. Click **Create App**.
3. Scroll down and locate the *Tracker* app template.
4. Hover the cursor over the *Tracker* app template and click **Select Template** > **Select This Template**.
5. Enter the name *Tracker*, choose a background color, an icon, and click **Create App**. Your app's *Buzz* space will open.
...
7. Create user interaction elements for deleting, searching, sorting, and filtering parts from the parts list:<br/>
   ...
   c. Add a column below the search box, go to the **Toolbox** tab > search for *sort* widget > drag and drop the **Drop down sort** widget inside the column.<br/>
...
3. Create and configure attributes:
   ...
   b. Select an attribute and go to the **Properties** tab to configure it.
...
2. Choose an entity for sourcing the data:<br/>
...
4. Create and configure a show message activity for displaying a save confirmation message:<br/>
   ...
   b. In the microflow diagram, select the **Show message** activity and go to the **Properties** tab > **Data Source** section > **Template** > enter *Part saved successfully* and turn the **Blocking** switch on. This activity sets an event to display the save confirmation message and block you from interacting with other user interaction elements on the *Parts* page.

The configured microflow may look like the following microflow. To understand more about the parameters and events displayed in the following microflow, refer [Microflows](https://docs.mendix.com/studio/microflows):

![](https://github.com/RaviBasagonda/mendix/blob/main/docs/images/6/microflow.png?raw=true)

### 6.4 Configuring the Edit Button

To configure the *Edit* button for editing the part details, follow these steps:

1. Click the **Edit** button, go to the **Properties** tab > **Events** section > **On Click Action** > choose **Page**.<br/>

{{% alert type="warning" %}}
In the **Events** section, do not turn the **Create Object** switch on. This event sets an action to create and add a new object (part) to the parts list, rather than changing the part detail that you want to edit from the parts list.
{{% /alert %}}<br/>
...
2. Choose the part attribute that you want to search using the search box:<br/>
...
3. Choose a placeholder to hint you that the part name can be used for searching:
...
You cannot preview and publish your app without fixing the error messages. If your app has errors, the Mendix Studio displays the number of errors in the red colored **Checks** button located at the top right corner.
